...
public class AgoraService {
  ...
  private RtmTokenBuilder2 rtmTokenBuilder2;
  ...
  public AgoraService(AgoraCredentials agoraCredentials, RtcTokenBuilder2 rtcTokenBuilder2,
      RtmTokenBuilder2 rtmTokenBuilder2) {
    this.agoraCredentials = agoraCredentials;
    this.rtcTokenBuilder2 = rtcTokenBuilder2;
    this.rtmTokenBuilder2 = rtmTokenBuilder2;
  }

  /**
   * Generates a rtc token for the user for the given channel
   ...
   */
  public String getRtcToken(Long existingUserId, String channelName) {
    // check if the channel name is not empty
    ...
    try {
      ...
      String errMsg = "Error while generating Agora RTC token: " + e.getMessage();
      log.error(errMsg);
  ...
  /**
   * Generates a rtm token for the user
   * @param existingUserId id of the user that exists (pre-condition)
   * @return RTM token for the user
   * @throws ResponseStatusException if the token cannot be generated (500 if env vars not set, 503
   *     if external service not available)
   */
  public String getRtmToken(Long existingUserId) {
    // check if the credentials are set (500 error if not)
    checkCredentialVariable(agoraCredentials.getAppId(), "AGORA_APP_ID");
    checkCredentialVariable(agoraCredentials.getAppCertificate(), "AGORA_APP_CERTIFICATE");
    ...
    // generate token or throw 503 error if external service not available
    try {
      log.info("Generating Agora RTM token for user with id {}", existingUserId);

      return rtmTokenBuilder2.buildToken(agoraCredentials.getAppId(),
          agoraCredentials.getAppCertificate(), String.valueOf(existingUserId), tokenExpirationTs);
    } catch (Exception e) {
      String errMsg = "Error while generating Agora RTM token: " + e.getMessage();
      log.error(errMsg);
      throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, errMsg);
    }
  }
