...
public class TaskControllerTest {
  ...
  public void createTask_missingFields_throwsError() throws Exception {
    ...
    given(taskService.createTask(Mockito.any()))
        .willThrow(new ResponseStatusException(HttpStatus.BAD_REQUEST, "Some needed fields are missing in the task object."));
    ...
    mockMvc.perform(postRequest)
        ...
        .andExpect(result
            -> assertTrue(
                result.getResolvedException().getMessage().contains("Some needed fields are missing in the task object.")));
  }
  ...
  public void createTask_unauthorizedAccess_throwsError() throws Exception {
      ...
      Mockito.doThrow(new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Not authorized to access."))
          .when(authorizationService).isAuthorized(Mockito.any());
  ...
  public void getTasks_noTasksInTeam_throwsError() throws Exception {
    ...
    given(taskService.getTasksByTeamId(Mockito.anyLong()))
        .willThrow(new ResponseStatusException(HttpStatus.NOT_FOUND, "No tasks found for team with id 1"));
    ...
    mockMvc.perform(getRequest)
        ...
        .andExpect(result
            -> assertTrue(
                result.getResolvedException().getMessage().contains("No tasks found for team with id 1")));
  }
